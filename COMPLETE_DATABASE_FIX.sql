...
-- QR Restaurant SaaS - Customer Flow Fix
-- ========================================

-- This script fixes ALL database issues for the customer QR scan flow
...
-- This is the core missing piece - we need a proper customers table
...
-- ========================================
-- 2. ADD MISSING CUSTOMER_ID TO ORDERS
-- ========================================
...
-- Add foreign key constraint to customers table (not auth.users)
...
-- ========================================
-- 4. FIX CUSTOMER_SESSIONS FOREIGN KEYS
-- ========================================

-- Fix customer_sessions foreign key constraint
DO $$
BEGIN
  -- Drop existing constraint if it exists
  IF EXISTS (SELECT 1 FROM information_schema.table_constraints 
...
-- ========================================
-- 5. FIX LOYALTY_POINTS FOREIGN KEYS
-- ========================================

-- Fix loyalty_points foreign key constraint
DO $$
BEGIN
  -- Drop existing constraint if it exists
  IF EXISTS (SELECT 1 FROM information_schema.table_constraints 
...
COMMENT ON TABLE public.customers IS 'Customer records for both guest (QR scan) and registered customers';
...
COMMENT ON COLUMN public.orders.customer_id IS 'Links to customers table for proper customer tracking';
COMMENT ON COLUMN public.customers.is_guest IS 'True for QR scan customers, false for registered users';
COMMENT ON COLUMN public.customers.auth_user_id IS 'Links to auth.users if guest customer registers later';

-- ========================================
-- 13. VERIFY SCHEMA INTEGRITY
-- ========================================
...
-- 4. Monitor performance logs for any remaining issues
